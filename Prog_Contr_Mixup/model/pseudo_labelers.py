...
class GraphLabelPropagation(torch.nn.Module):
    ...
    def forward(self,
            ...
            idxs: np.array,
            cls_pseudo_labels: np.array,
            cls_pseudo_labels_conf):
        ...
        #-------------------------------------
        agree = None #(p_labels==cls_pseudo_labels)
        # weights = agree
        # agr_idx = np.where(agree)
        # wpl = np.where(p_labels[agr_idx]!=labels[agr_idx])
        # weights = agree
        #--------------------------------
        # p_labels *= mask
        #import pdb; pdb.set_trace()
        #weights = (weights+cls_pseudo_labels_conf)/2
        #weights = agree
        
        # weights = (p_labels==cls_pseudo_labels).astype(np.float)
        # acc = agree.mean()#((agree * p_labels)==labels).mean()
        # cls_acc = (cls_pseudo_labels==labels).mean()
        # acc = cls_acc
        #-----------------------------------
        # Compute the accuracy of pseudolabels for statistical purposes       
        # correct_idx = (p_labels==labels) #(p_labels == labels)==cls_pseudo_labels
        # acc = correct_idx.mean()
        
        #agree = (p_labels==cls_pseudo_labels)
        #arg9w_idx = np.where(weights>=0.9)[0]
        #final_idx = arg9w_idx[np.where(agree[arg9w_idx])[0]] #agreement_weight>0.9
        #idxs_2_zero = list(set(np.arange(len(weights)))-set(final_idx)) #other weights to be 0
        # weights[idxs_2_zero] = 0 # set weights to 0 for not in agreement+weight>0.9 


        p_labels[labeled_idx] = labels[labeled_idx]
        weights[labeled_idx] = 1.0
        # so far I have some pseudo-labels with weights >0.9
        ...
        acc = (p_labels[weights>0]==labels[weights>0]).mean()
        ...
        return self.p_labels, acc, agree, probs_l1
