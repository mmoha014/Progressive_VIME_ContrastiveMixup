...
def main(config, args,  parameters=None, seed=None, pretrain=None):
    ...
    if (pretrain):
        ...
        model = config.init_obj('unsupervised_arch', module_arch,
                    cat_idxs=getattr(data_loader.dataset, 'cat_idxs',[]),#[1, 3, 5, 6, 7, 8, 9, 13]),
                    cat_dims=getattr(data_loader.dataset, 'cat_dims', []),#[9, 16, 7, 15, 6, 5, 2, 42]),
                    cont_idxs=getattr(data_loader.dataset, 'cont_idxs', []),#[0, 2, 4, 10, 11, 12]),
                    )
        logger.info(model)
        ...
        if hasattr(trainer, 'pseudo_labeler'):
            nni_dict['pseudo_labeling_acc'] = trainer.data_loader.update_pseudo_labels(trainer.model, trainer.pseudo_labeler, config["unsupervised_trainer"]['pseudolabeling']['update_cpr'], trainer.device)
    ...
    if model is not None:
        if True: #'unsupervisd_model_load_best' in config and config['unsupervised_model_load_best']:
    ...
    if True: #'supervised_model_best' in config and config['supervised_model_best']:
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default='./configs/TrafficVio/contrastivemixup.json', type=str,
    ...
    args.add_argument('--pretrain', '-pretrain', default=True, action='store_true',
    ...
    if (args.ax_config):
        ...
        main(config, args)
#check accuracy of classifier + how weights are used+train a network alone
